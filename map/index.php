...
<body>
  ...
  <a href="https://www.piersoft.it/daebot/" target="_blank"><img src="logo.png" width="40px" title="localizzami" alt="localizzami"></a>
  </div>
  <script>
	  var attr_osm = 'Map data &copy; <a href="http://openstreetmap.org/">OpenStreetMap</a> contributors. DAE Map. Powered by <a href="https://www.twitter.com/piersoft/">@piersoft</a> |<a href="http://www.piersoft.it/?p=1045"> INFO</a>',
    ...
    var lat=parseFloat('<?php printf($_GET['lat']); ?>'),
        lon=parseFloat('<?php printf($_GET['lon']); ?>'),
          zoom=14;
		var map = new L.Map('map').addLayer(osm).setView(new L.LatLng(lat,lon), 17);
    var markeryou = L.marker([parseFloat('<?php printf($_GET['lat']); ?>'), parseFloat('<?php printf($_GET['lon']); ?>')],{
        draggable: true
    }).addTo(map);
    markeryou.bindPopup("<b>Sei qui</b>");

    markeryou.on("drag", function(e) {
        var marker = e.target;
        var position = marker.getLatLng();
        window.open("index.php?lat="+position.lat+"&lon="+position.lng,"_self");
    ...
    function Dist(lat1, lon1, lat2, lon2)
      ...
      var d = R * c;
  var d1=d.toFixed(3)+"";
if (d1 < 1) d1=(d*1000).toFixed(3)+" mt";
else d1=d+" km";
      return d1;                      //Ritorno 3 decimali
    }
