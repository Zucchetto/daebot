...
//	$telegram->answerInlineQuery($content);
	...
	foreach ($osm_data_dec->node as $osm_element) {
		...
		$map ="http://www.piersoft.it/daebot/map/index.php?lat=".$osm_element['lat']."&lon=".$osm_element['lon'];
...
\n\nBot made by @piersoft and @iltempe.\nTutorial: https://goo.gl/UCcwkw\nVideo BLSD procedure: https://www.youtube.com/watch?v=vnS_PNh0oDk";
	...
	function location_manager($db,$telegram,$user_id,$chat_id,$location)
		{
			...
			$telegram->sendMessage($content);
				...
				//	$mappa="https://www.openstreetmap.org/?mlat=".$osm_element['lat']."&mlon=".$osm_element['lon']."#map=19/".$osm_element['lat']."/".$osm_element['lon'];
					$mappa ="http://www.piersoft.it/daebot/map/index.php?lat=".$osm_element['lat']."&lon=".$osm_element['lon'];
